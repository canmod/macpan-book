...
* `Type` -- One of the following strings:
  ...
  * `"rel_orig_logit"` -- The logit transform of the `Value` column is added to the logit transform of the original value, and then the inverse logit transform is applied to this sum to generate a new value for the parameter on `Date`
  * `"rel_prev_logit"` -- The logit transform of the `Value` column is added to the logit transform of the previous value of the changing parameter, and then the inverse logit transform is applied to this sum to generate a new value for the parameter on `Date`
...
| `rel_orig_logit` | $\theta_\tau = \text{logit}^{-1}\left(\text{logit}(\theta_0) + \text{logit}(\nu_\tau)\right)$ |
| `rel_prev_logit` | $\theta_\tau = \text{logit}^{-1}\left(\text{logit}(\theta_{\tau-1}) + \text{logit}(\nu_\tau)\right)$ |
