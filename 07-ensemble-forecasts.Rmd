...
set.seed(1L)
synthetic_data = (sir_with_obs_err
...
sir_to_calibrate = (sir_with_obs_err
  %>% update_observed(
    synthetic_data, 
    loss_params = sir_with_obs_err$observed$loss_params
  )
  %>% update_opt_params(
    log_beta ~ log_flat(0),
...
```{r extend_end_date}
sir_to_forecast = extend_end_date(
  sir_calibrated, 
  days_to_extend = 100
)
```
...
```{r simulate_ensemble)}
sir_forecasts = (simulate_ensemble(
    sir_to_forecast, 
    use_progress_bar = FALSE
  )
  %>% filter(name != "S_to_I")
)
rmarkdown::paged_table(sir_forecasts)
```
...
```{r join_obs_data_to_sims}
sir_fits = (sir_forecasts
  %>% left_join(
    synthetic_data, 
    c("Date" = "date", "name" = "var"), 
    suffix = c("_fitted", "")
  )
  %>% mutate(
    name = factor(
      name, 
      levels = topological_sort(sir_to_forecast))
  )
)
(ggplot(sir_fits)
